...
/*app.get('/appointment',  authController.isLoggedIn, (req, res, fields)=> {
    con.query('SELECT * FROM appointment WHERE patient = ?', (err, rows)=>{
        if(err) throw err;
            console.log(rows);
            res.render('appointment', {student: req.student, title:"Class Cases", requests: rows}); 
    })
});*/

app.get('/appt', authController.isLoggedIn, function(req,res,next) {
    con.query("SELECT * FROM appointment WHERE patient = ?", [req.student.computing_id], function(err, rows, fields){
        if (err) throw err;
        console.log(rows);
        res.render('appt', {student: req.student, title:"My Appointment Requests", requests: rows}); 
    })
})
...
app.post('/rate/:id', (req, res, fields)=> {
    const id = req.body.appointment_id;
    const numeric = req.body.numeric_rating;
